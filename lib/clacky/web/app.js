...
const Router = (() => {
  ...
  function _parseHash(hash) {
    ...
    const wm = h.match(/^workspace(?:\/(.+))?$/);
    if (wm)                           return { view: "workspace", params: { sub: wm[1] || null } };
  ...
  function _apply(view, params = {}) {
    ...
    switch (view) {
      ...
      case "workspace": {
        // workspace hash is owned by workspace.js; Router just shows the panel
        const sub = params.sub || null;
        const hash = sub ? `workspace/${sub}` : "workspace";
        _setHash(hash);
        const wp = $("workspace-panel");
        if (wp) { wp.style.display = "flex"; wp.style.flexDirection = "column"; }
        Sessions.renderList();
        // Fire a hashchange-like event so workspace.js can handle the route
