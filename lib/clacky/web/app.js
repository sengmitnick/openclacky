...
const Router = (() => {
  ...
  // Hide all panels (including dynamically injected plugin panels).
  function _hideAll() {
    PANELS.forEach(p => {
      const el = $(p);
    ...
    document.querySelectorAll(".plugin-panel").forEach(el => {
  ...
  function _parseHash(hash) {
    ...
    // Support both #plugin/:id  and  #plugin/:id/:sub
    const pm = h.match(/^plugin\/([^/]+)(?:\/(.+))?$/);
    if (pm)                           return { view: "plugin",   params: { id: pm[1], sub: pm[2] || null } };
  ...
  // Plugin sidebar items are registered dynamically via registerPlugin().
  const SIDEBAR_ITEMS = {
  ...
  // Remove active highlight from all Router-managed sidebar items (including plugins).
  function _clearSidebarActive() {
    Object.values(SIDEBAR_ITEMS).forEach(id => {
      const el = $(id);
      if (el) el.classList.remove("active");
    });
    // Clear plugin sidebar entries
    document.querySelectorAll(".plugin-sidebar-entry").forEach(el => {
  ...
  function _apply(view, params = {}) {
    ...
    switch (view) {
      ...
      case "plugin": {
        // Plugin panel: show the injected panel for the given plugin id
        // Supports sub-routes: #plugin/:id  and  #plugin/:id/:sub
        const pluginId    = params.id;
        const pluginSub   = params.sub || null;  // e.g. a project id
        const pluginPanel = Plugins.getPanel(pluginId);
        if (!pluginPanel) {
          _apply("welcome");
          return;
        }
        // Write hash including sub-route when present
        _setHash(pluginSub ? `plugin/${pluginId}/${pluginSub}` : `plugin/${pluginId}`);
        pluginPanel.style.display = "flex";
        pluginPanel.style.flexDirection = "column";

        // Highlight plugin sidebar entry (the top-level divider/entry)
        const sidebarItem = Plugins.getSidebarItem(pluginId);
        if (sidebarItem) sidebarItem.classList.add("active");

        // Notify plugin scripts that navigation has occurred (pass sub as part of params)
        Plugins.notifyNavListeners("plugin", params);
...
// ── Plugin system boot ────────────────────────────────────────────────────
// Initialize plugins immediately so sidebar entries & panels are ready
// before the first Router.restoreFromHash() call.
Plugins.init();
...
function _renderAttachmentPreviews() {
  ...
  // Render file badges (PDF, ZIP, DOC, XLS, PPT, etc.)
...
function sendMessage() {
  ...
  const files = [
    ...
    ..._pendingFiles.map(f => ({
      ...
      path:         f.path,
      preview_path: f.preview_path || null
    }))
  ];
