...
function _addPdfFile(file) {
  if (file.size > MAX_PDF_SIZE) {
    alert(`PDF too large: ${file.name} (max 32 MB)`);
    return;
  }
  ...
  fetch("/api/upload", { method: "POST", body: formData })
    ...
    .then(data => {
      ...
      _pendingPdfs.push({ file_id: data.file_id, name: data.name, path: data.path });
      _renderAttachmentPreviews();
    })
...
function _renderAttachmentPreviews() {
  ...
  strip.innerHTML = "";
  const hasContent = _pendingImages.length > 0 || _pendingPdfs.length > 0;
  if (!hasContent) {
...
function sendMessage() {
  ...
  if (!content && _pendingImages.length === 0 && _pendingPdfs.length === 0) return;
  ...
  if (_pendingPdfs.length > 0) {
    const badges = _pendingPdfs
      .map(pdf => `📄 <em>${escapeHtml(pdf.name)}</em>`)
      .join(" ");
  ...
  const files  = _pendingPdfs.map(pdf => ({
    file_id:   pdf.file_id,
    name:      pdf.name,
...
const SkillAC = (() => {
  ...
  function _openAll() {
    _activeIndex = -1;  // No default selection, only highlight on hover
    _render("");
    $("user-input").focus();
  }
  ...
  return {
    ...
    update(value) {
      ...
      // No default selection when typing, only highlight on hover/arrow keys
      _activeIndex = -1;
      _render(query);
    },
