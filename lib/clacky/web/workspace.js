...
(() => {
  ...
  const _CA_STRINGS = {
    en: {
      title: "Dashboard", subtitle: "Overview of all project tasks",
      newProject: "New Project", labelName: "Project Name",
      ...
      statTotal: "Total Tasks", statTotalDesc: "across all projects",
      statRunning: "Running", statRunningDesc: "tasks in progress",
      statReview: "Awaiting Review", statReviewDesc: "waiting for your review",
      statDone: "Completed", statDoneDesc: "all completed tasks",
      sectionProjects: "My Projects", sectionProjectsSubtitle: "Quick access to your projects",
    },
    zh: {
      title: "仪表盘", subtitle: "概览所有项目的任务状态",
      newProject: "新建项目", labelName: "项目名称",
      ...
      statTotal: "总任务数", statTotalDesc: "跨所有项目",
      statRunning: "进行中", statRunningDesc: "正在执行的任务",
      statReview: "待审查", statReviewDesc: "等待您审查",
      statDone: "已完成", statDoneDesc: "所有已完成任务",
      sectionProjects: "我的项目", sectionProjectsSubtitle: "快速访问您的项目",
    },
  };
  ...
  // ── Stats loading ──────────────────────────────────────────────────────
  async function _loadStats(projects) {
    // Aggregate stats from all projects' tasks
    let total = 0, running = 0, review = 0, done = 0;
    try {
      const results = await Promise.allSettled(
        projects.map(p => fetch(`/api/projects/${p.id}/tasks`).then(r => r.ok ? r.json() : { tasks: [] }))
      );
      results.forEach(r => {
        ...
        tasks.forEach(t => {
          ...
          if (st === "running" || st === "active") running++;
          else if (st === "review" || st === "waiting_review" || st === "pending_review") review++;
          else if (st === "done" || st === "completed" || st === "closed") done++;
        });
      });
    } catch (_) {}

    const set = (id, val) => { const el = _$(id); if (el) el.textContent = String(val); };
    set("ca-stat-num-total",   total);
    set("ca-stat-num-running", running);
    set("ca-stat-num-review",  review);
    set("ca-stat-num-done",    done);

    // Update total desc with project count
    const descEl = _$("ca-stat-desc-total");
    if (descEl) {
      const lang = (typeof I18n !== "undefined") ? I18n.lang() : "en";
      if (lang === "zh") {
        descEl.textContent = `跨 ${projects.length} 个项目`;
      } else {
        descEl.textContent = `across ${projects.length} project${projects.length !== 1 ? "s" : ""}`;
      }
    }
  }
  ...
  function renderGrid(projects) {
    ...
    if (projects.length === 0) {
      ...
      // Zero out stats
      ["ca-stat-num-total","ca-stat-num-running","ca-stat-num-review","ca-stat-num-done"].forEach(id => {
        const el = _$(id); if (el) el.textContent = "0";
      });
      return;
    }
    grid.innerHTML = projects.map(p => `
      <div class="ca-project-card" data-ca-id="${p.id}">
        <div class="ca-project-card-top">
          <div class="ca-project-icon">
        ...
        <div class="ca-project-footer">
          <span class="ca-project-date">${_fmtDate(p.created_at)}</span>
          <span class="ca-project-arrow">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
          </span>
        </div>
      </div>
    `).join("");
    ...
    grid.querySelectorAll(".ca-project-card").forEach(card => {
      card.addEventListener("click", async e => {
        if (e.target.closest("[data-ca-delete]")) return;
  ...
  function applyPanelI18n() {
    ...
    set("ca-subtitle-projects", _t("subtitle"));
    set("ca-new-proj-label",   _t("newProject"));
    ...
    // Stats labels
    set("ca-stat-label-total",   _t("statTotal"));
    set("ca-stat-desc-total",    _t("statTotalDesc"));
    set("ca-stat-label-running", _t("statRunning"));
    set("ca-stat-desc-running",  _t("statRunningDesc"));
    set("ca-stat-label-review",  _t("statReview"));
    set("ca-stat-desc-review",   _t("statReviewDesc"));
    set("ca-stat-label-done",    _t("statDone"));
    set("ca-stat-desc-done",     _t("statDoneDesc"));
    set("ca-section-projects-title",    _t("sectionProjects"));
    set("ca-section-projects-subtitle", _t("sectionProjectsSubtitle"));
  ...
  function _injectPanelHTML() {
    ...
    el.innerHTML = `
      ...
      <!-- Dashboard view -->
      <div class="ca-panel ca-dashboard-scroll" id="ca-view-dashboard">
        <div class="ca-dashboard-inner">
          <!-- Header row: title + action buttons -->
          <div class="ca-panel-header">
            <div class="ca-panel-title-group">
              <h2 class="ca-panel-title" id="ca-title-projects"></h2>
              <p class="ca-panel-subtitle" id="ca-subtitle-projects"></p>
            </div>
            <div class="ca-panel-actions">
              <button class="ca-btn ca-btn-primary ca-new-proj-btn" id="ca-new-project-btn">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" style="width:16px;height:16px;flex-shrink:0"><path d="M12 10v6"/><path d="M9 13h6"/><path d="M20 20a2 2 0 0 0 2-2V8a2 2 0 0 0-2-2h-7.9a2 2 0 0 1-1.69-.9L9.6 3.9A2 2 0 0 0 7.93 3H4a2 2 0 0 0-2 2v13a2 2 0 0 0 2 2Z"/></svg>
                <span id="ca-new-proj-label"></span>
              </button>
            </div>
          </div>
          ...
          <!-- Stats cards row -->
          <div class="ca-stats-grid" id="ca-stats-grid">
            <div class="ca-stat-card ca-stat-card-total">
              <div class="ca-stat-card-deco"></div>
              <div class="ca-stat-card-inner">
                <div class="ca-stat-card-header">
                  <span class="ca-stat-label" id="ca-stat-label-total"></span>
                ...
                <div class="ca-stat-number" id="ca-stat-num-total">—</div>
                <p class="ca-stat-desc" id="ca-stat-desc-total"></p>
              </div>
            </div>
            <div class="ca-stat-card ca-stat-card-running">
              <div class="ca-stat-card-deco"></div>
              <div class="ca-stat-card-inner">
                <div class="ca-stat-card-header">
                  <span class="ca-stat-label" id="ca-stat-label-running"></span>
                  <svg class="ca-stat-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M9 9.003a1 1 0 0 1 1.517-.859l4.997 2.997a1 1 0 0 1 0 1.718l-4.997 2.997A1 1 0 0 1 9 14.996z"/><circle cx="12" cy="12" r="10"/></svg>
                </div>
                <div class="ca-stat-number" id="ca-stat-num-running">—</div>
                <p class="ca-stat-desc" id="ca-stat-desc-running"></p>
              </div>
            </div>
            <div class="ca-stat-card ca-stat-card-review">
              <div class="ca-stat-card-deco"></div>
              <div class="ca-stat-card-inner">
                <div class="ca-stat-card-header">
                  <span class="ca-stat-label" id="ca-stat-label-review"></span>
                  <svg class="ca-stat-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m21 21-4.34-4.34"/><circle cx="11" cy="11" r="8"/></svg>
                </div>
                <div class="ca-stat-number" id="ca-stat-num-review">—</div>
                <p class="ca-stat-desc" id="ca-stat-desc-review"></p>
              </div>
            </div>
            <div class="ca-stat-card ca-stat-card-done">
              <div class="ca-stat-card-deco"></div>
              <div class="ca-stat-card-inner">
                <div class="ca-stat-card-header">
                  <span class="ca-stat-label" id="ca-stat-label-done"></span>
                  <svg class="ca-stat-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><path d="m9 12 2 2 4-4"/></svg>
                </div>
                <div class="ca-stat-number" id="ca-stat-num-done">—</div>
                <p class="ca-stat-desc" id="ca-stat-desc-done"></p>
              </div>
            </div>
          </div>
          ...
          <div class="ca-section-header">
            <div>
              <h3 class="ca-section-title" id="ca-section-projects-title"></h3>
              <p class="ca-section-subtitle" id="ca-section-projects-subtitle"></p>
            </div>
          </div>
